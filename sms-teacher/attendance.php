...
date_default_timezone_set('Africa/Lagos'); // Ensure correct date 
...
class AttendanceData {
     ...
     * Includes classes specifically assigned to this teacher (Class Teacher).
     */
    public function getTeacherClasses(): array {
        ...
        $query = "
            SELECT DISTINCT
                ...
                c.class_code
            FROM classes c
            LEFT JOIN class_subjects cs ON c.id = cs.class_id AND cs.teacher_id = :teacher_id
            WHERE cs.teacher_id IS NOT NULL 
               OR c.class_teacher_id = :teacher_id
    ...
    public function getStudentsForClass(int $class_id, string $attendance_date): array {
        ...
        try {
            ...
            // 2. Calculate overall attendance rate for each student
            $today = date('Y-m-d');
            $stmt_rate = $this->conn->prepare("
                SELECT 
            ...
            foreach ($students as &$student) {
                ...
                $att_data = $stmt_rate->fetch();
                
                if ($att_data['total_records'] > 0) {
                     $student['attendance_rate'] = round(($att_data['present_count'] / $att_data['total_records']) * 100);
                } else {
                ...
                if (empty($student['current_status'])) {
                    $student['current_status'] = ''; // LEAVE EMPTY
                }
            }
    ...
    public function saveAttendance(int $class_id, string $attendance_date, array $attendance_records): bool {
        ...
        $day_of_week = date('l', strtotime($attendance_date));
        if (in_array($day_of_week, ['Saturday', 'Sunday'])) {
            $this->error_message = "Cannot take attendance on weekends (Saturday and Sunday).";
            return false;
        }
        ...
        try {
            ...
            // Fetch current session and term with proper sync
            $academic_session_id = null; 
            $term_id = null;
            
            $stmt_session = $this->conn->query("SELECT id FROM academic_sessions WHERE is_current = 1 LIMIT 1");
            $academic_session_id = $stmt_session->fetchColumn();
            
            $stmt_term = $this->conn->query("SELECT id FROM terms WHERE is_current = 1 LIMIT 1");
            $term_id = $stmt_term->fetchColumn();
            
            // If no current term found, trigger sync
            if (!$academic_session_id || !$term_id) {
                require_once '../sync_calendar.php';
                $syncResult = syncTermsWithCalendar();
                if ($syncResult['success']) {
                    // Retry getting current session and term
                    $academic_session_id = $this->conn->query("SELECT id FROM academic_sessions WHERE is_current = 1 LIMIT 1")->fetchColumn();
                    $term_id = $this->conn->query("SELECT id FROM terms WHERE is_current = 1 LIMIT 1")->fetchColumn();
                }
            }
            ...
            $insert_query = "
                ...
                VALUES (:student_id, :class_id, :attendance_date, :status, :recorded_by, :session_id, :term_id) 
            ";
...
// Ensure we're using the current date if no specific date is requested
if (!isset($_GET['date'])) {
    $attendance_date = date('Y-m-d'); // Always use today's date as default
}
...
function generateCalendar($year_month, $attendance_days, $current_date, $class_id) {
    ...
    for ($day = 1; $day <= $daysInMonth; $day++) {
        ...
        $is_weekend = ($day_of_week == 0 || $day_of_week == 6); // Sunday or Saturday
        
        if ($is_weekend) {
            $output .= "
            <div class='calendar-day text-gray-400 bg-gray-100 cursor-not-allowed text-sm' title='Weekend - No school'>
                {$day}
            </div>";
        } elseif ($is_future) {
            $output .= "
...
<?php endif; ?>
        ...
        <div id="page-content">
            <span id="page-meta-data" data-title="Attendance" hidden></span>
            ...
            <!-- Attendance Content -->
            <div class="p-4 md:p-6 transition-all duration-300">
                <div class="bg-white rounded-xl shadow-md p-4 md:p-6">
                    <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-4 md:mb-6 space-y-3 md:space-y-0">
                        ...
                        <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4">
                            ...
                            <button type="button" onclick="goToToday()" class="px-3 py-2 bg-blue-500 text-white rounded-lg text-sm hover:bg-blue-600">
                                Today
                            </button>
                        </div>
                    </div>
                    ...
                    <!-- Action Buttons -->
                    <div class="flex flex-col sm:flex-row gap-3 mb-6 pb-6 border-b">
                        <button id="quickAttendanceBtn" class="bg-nskblue text-white px-4 py-2.5 rounded-lg hover:bg-nsknavy transition text-sm font-semibold flex items-center justify-center">
                            <i class="fas fa-bolt mr-2"></i>Quick Mark All Present
                        </button>
                        <button id="saveAttendanceBtn" class="bg-nskgreen text-white px-4 py-2.5 rounded-lg hover:bg-green-600 transition text-sm font-semibold flex items-center justify-center">
                                ...
                                <?php if ($class_id && count($students) > 0): ?>
                                    <?php foreach ($students as $student): ?>
                                        <?php 
                                            ...
                                            $rate_color_hex = '#10b981'; // Green (default)
                                            if (is_numeric($rate)) {
                                                if ($rate < 80) $rate_color_hex = '#ef4444'; // Red
                                                elseif ($rate < 90) $rate_color_hex = '#f59e0b'; // Amber
                                            } else {
                                                $rate_color_hex = '#6b7280'; // Gray for N/A
                                            }
                                        ...
                                        <tr class="hover:bg-gray-50" data-student-id="<?= $student['student_id'] ?>">
                                            <td class="py-4 px-3 md:px-6">
                                                <div class="flex items-center">
                                                    ...
                                                    <div class="ml-3">
                                                        ...
                                                        <p class="text-xs text-gray-600">ID: <?= htmlspecialchars($student['admission_id'] ?? $student['student_id']) ?>
                                                            <span class="ml-2 px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">
                                                                <?php if (is_numeric($student['attendance_rate'])): ?>
                                            ...
                                            <td class="py-4 px-3 md:px-6">
                                                <div class="flex justify-center space-x-2">
                                                    <button class="text-nskblue hover:text-nsknavy view-history-btn transition" 
                                                            data-student-name="<?= htmlspecialchars($student['first_name'] . ' ' . $student['last_name']) ?>"
                                                            data-student-id="<?= $student['student_id'] ?>"
                                                            title="View Attendance History">
                                                        <i class="fas fa-history"></i>
                                                    </button>
                                                    <button class="text-nskgreen hover:text-green-700 add-note-btn transition" 
                                                            data-student-name="<?= htmlspecialchars($student['first_name'] . ' ' . $student['last_name']) ?>"
                                                            data-student-id="<?= $student['student_id'] ?>"
                                                            title="Add Note">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                    ...
                    <div class="bg-white rounded-xl shadow-md p-4 md:p-6">
                        <div class="flex justify-between items-center mb-4">
                            <button onclick="changeMonth(-1)" class="text-nsknavy hover:text-nskblue">
                                <i class="fas fa-chevron-left"></i>
                            </button>
                            <h3 class="text-lg md:text-xl font-bold text-nsknavy" id="calendarMonthTitle"><?= date('F Y', strtotime($attendance_date)) ?></h3>
                            <button onclick="changeMonth(1)" class="text-nsknavy hover:text-nskblue">
                    ...
                    <div class="bg-white rounded-xl shadow-md p-4 md:p-6">
                        ...
                        <div class="space-y-4">
                            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                ...
                                <button onclick="downloadWeeklyReport()" class="bg-nskblue text-white px-3 py-1 rounded text-xs hover:bg-nsknavy transition">
                            ...
                            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                ...
                                <button onclick="downloadMonthlyReport()" class="bg-nskblue text-white px-3 py-1 rounded text-xs hover:bg-nsknavy transition">
                                    <i class="fas fa-download mr-1"></i>Download
                                </button>
                            </div>
                            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                ...
                                <button onclick="generateStudentReport()" class="bg-nskblue text-white px-3 py-1 rounded text-xs hover:bg-nsknavy transition">
            ...
            <script>
                // --- PHP DATA ---
                // --- PHP DATA ---
                let CURRENT_CLASS_ID = <?= json_encode($class_id) ?>;
                let CURRENT_DATE = <?= json_encode($attendance_date) ?>;
                let TOTAL_STUDENTS = <?= count($students) ?>;

                // Create and Attach DOM Search Listener
                const searchInput = document.getElementById('globalSearch');
                if (searchInput) {
                    searchInput.addEventListener('input', function(e) {
                        const term = e.target.value.toLowerCase();
                        const rows = studentAttendanceBody.querySelectorAll('tr');
                        
                        rows.forEach(row => {
                            // Check text content of the row (Name and ID)
                ...
                function changeFilter() {
                    ...
                    const url = `attendance.php?class_id=${classId}&date=${date}`;
                ...
                function changeMonth(delta) {
                    ...
                    // Get the current day of the month before changing
                    const currentDay = date.getDate();
                    
                    // Change the month
                    date.setMonth(date.getMonth() + delta);
                    
                    // JavaScript automatically handles month overflow (e.g., Jan 31 + 1 month = Mar 3)
                    // But we want to keep the same day or use the last valid day of the target month
                    // Check if the day changed unexpectedly (overflow occurred)
                    if (date.getDate() !== currentDay) {
                        // Set to the last day of the previous month (the target month)
                        date.setDate(0);
                    }
                    
                    const newDate = date.toISOString().split('T')[0];
                    
                    // Update input value (which triggers nothing automatically unless we set it)
                    attendanceDate.value = newDate;
                    
                    // Trigger filter change directly
                    changeFilter();
                }
                ...
                // --- Event Listeners Setup ---
                {
                    ...
                    // Note buttons (delegated event listener for Add Note)
                    studentAttendanceBody.addEventListener('click', (e) => {
                    ...
                    // View Student History button (separate from Add Note)
                    studentAttendanceBody.addEventListener('click', (e) => {
                ...
                function downloadWeeklyReport() {
                    ...
                    window.open(`download_attendance_report.php?class_id=${classId}&type=weekly&start_date=${startDate}&end_date=${endDate}`, '_blank');
                }
                ...
                function downloadMonthlyReport() {
                    ...
                    window.open(`download_attendance_report.php?class_id=${classId}&type=monthly&year=${year}&month=${month}`, '_blank');
                }
                ...
                function generateStudentReport() {
                    ...
                    // Show a prompt to select individual student or all students
                    Swal.fire({
                        title: 'Generate Reports?',
                        text: 'Generate attendance reports for all students in this class?',
                        icon: 'question',
                        ...
                        if (result.isConfirmed) {
                            window.open(`download_attendance_report.php?class_id=${classId}&type=student&all=true`, '_blank');
                        } else {
                            Swal.fire('Info', 'Please use the "View History" button next to a student to view their individual attendance report', 'info');
                        }
                    });
                }
