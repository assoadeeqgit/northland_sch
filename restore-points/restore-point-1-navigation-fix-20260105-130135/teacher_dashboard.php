    ...
    public function getQuickStats(): array
    {
        ...
        try {
            ...
            if (!$is_weekend) {
                ...
                $stmt3->bindParam(':session_id', $currentSessionId);
                $stmt3->bindParam(':term_id', $currentTermId);
                $stmt3->execute();
    ...
    public function getClassSummaries(): array
    {
        ...
        try {
            ...
            foreach ($raw_summaries as $summary) {
                ...
                $q_grade = "
                    SELECT AVG(r.total_score) as avg_marks
                    FROM student_results r
    ...
    <main class="main-content">
        ...
        <header class="desktop-header bg-white shadow-md p-4">
            <div class="flex justify-between items-center">
                ...
                <div class="flex items-center space-x-4">
                    <div class="relative">
                        <form method="GET" action="my_students.php" class="flex items-center space-x-2 bg-nsklight rounded-full py-2 px-4 border border-transparent focus-within:border-nskblue transition-colors">
                            <i class="fas fa-search text-gray-500"></i>
                            <input type="text" name="search" placeholder="Search students..." class="bg-transparent outline-none w-32 md:w-64 text-sm text-nsknavy">
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            console.log('Dashboard loaded successfully');
                                    ...
                                    <div class="flex items-center justify-between">
                                        <div>
                                            <p class="text-sm font-bold text-nsknavy">${student.name}</p>
                                            <p class="text-xs text-gray-500">${student.class} • ${student.adm}</p>
                                        </div>
                                        <i class="fas fa-chevron-right text-gray-300 text-xs"></i>
                                    </div>
                                `;
                                item.addEventListener('click', () => {
                                    // Navigate to My Students filtered by this student's name/ID
                                    // Ideally, my_students.php should support opening a specific student modal.
                                    // For now, filtering the table is good.
                                    window.location.href = `my_students.php?search=${encodeURIComponent(student.name)}`;
                                });
                            ...
                            resultsContainer.innerHTML = `
                                <div class="p-3 text-sm text-gray-500 text-center">No matches found</div>
                            `;
                        }
                    })
                    .catch(err => {
