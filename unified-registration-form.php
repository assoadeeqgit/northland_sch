...
<body class="min-h-screen flex items-center justify-center p-4">
    ...
    <script>
        ...
        const AuthManager = {
            ...
            roleFields: {
                student: [
                    { type: 'date', id: 'dateOfBirth', label: 'Date of Birth', required: true },
                    { type: 'select', id: 'gender', label: 'Gender', options: ['Male', 'Female', 'Other'], required: true },
                    { type: 'select', id: 'classLevel', label: 'Class Level', options: ['JSS 1', 'JSS 2', 'JSS 3', 'SS 1', 'SS 2', 'SS 3'], required: true },
                    { type: 'text', id: 'parentName', label: 'Parent/Guardian Name', required: true },
                    { type: 'tel', id: 'parentPhone', label: 'Parent/Guardian Phone', required: true },
                    { type: 'text', id: 'previousSchool', label: 'Previous School', required: false }
                ],
                teacher: [
                    { type: 'text', id: 'qualification', label: 'Highest Qualification', required: true },
                    { type: 'text', id: 'subjects', label: 'Subject Specialization', required: true },
                    { type: 'select', id: 'experience', label: 'Years of Experience', options: ['0-2 years', '3-5 years', '6-10 years', '10+ years'], required: true },
                    { type: 'select', id: 'department', label: 'Department', options: ['Science', 'Arts', 'Commercial', 'Technical'], required: true },
                    { type: 'select', id: 'employmentType', label: 'Employment Type', options: ['Full-time', 'Part-time', 'Contract'], required: true }
                ],
                parent: [
                    { type: 'select', id: 'relationship', label: 'Relationship to Student', options: ['Father', 'Mother', 'Guardian', 'Other'], required: true },
                    { type: 'text', id: 'studentId', label: 'Student ID (if existing)', required: false },
                    { type: 'text', id: 'occupation', label: 'Occupation', required: true },
                    { type: 'textarea', id: 'address', label: 'Home Address', required: true },
                    { type: 'text', id: 'emergencyContact', label: 'Emergency Contact', required: true }
                ],
                staff: [
                    { type: 'select', id: 'department', label: 'Department', options: ['Administration', 'Maintenance', 'Security', 'Kitchen', 'Library'], required: true },
                    { type: 'text', id: 'position', label: 'Job Title/Position', required: true },
                    { type: 'select', id: 'employmentType', label: 'Employment Type', options: ['Full-time', 'Part-time', 'Contract'], required: true },
                    { type: 'text', id: 'supervisor', label: 'Supervisor Name', required: false }
                ],
                admin: [
                    { type: 'select', id: 'adminLevel', label: 'Admin Level', options: ['Super Admin', 'Admin', 'Sub-Admin'], required: true },
                    { type: 'text', id: 'department', label: 'Department Access', required: true },
                    { type: 'textarea', id: 'permissions', label: 'Special Permissions', required: false }
                ]
            },
            ...
            handleSignIn() {
                ...
                setTimeout(() => {
                    ...
                    // Redirect based on role (simulate)
                    setTimeout(() => {
